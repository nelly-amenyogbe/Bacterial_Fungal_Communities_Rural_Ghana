...
# ITS2: Shared OTUs between mother-infant pairs
######################
...
p.inf <- filter(pairs, as.character(stdyid) %in% inf.samples$stdyid) # 14 infant samples
# choose only those where the partner is also present
p.bm <- filter(p.inf, partna %in% bm.samples$stdyid) # only 8 pairs present
bm.pairs <- unique(as.character(p.bm$pair.id))

p.ms <- filter(p.inf, partna %in% ms.samples$stdyid) # only 7 pairs present
...
# how many taxa?
length(unique(bm.shared$Genus)) # 11 Genera
table(as.character(bm.shared$Genus)) # aspergillus, candida...
...
# how many taxa?
length(unique(st.shared$Genus)) # 9 Genera
table(as.character(st.shared$Genus)) # aspergillus, candida, ascomycota...
...
select <- sample(c(1:244), size = 9, replace = FALSE)
...
test.groups <- unique(as.character(bm.inf.test$group.ages), na.rm = TRUE)
...
ggplot(bm.inf.test, aes(x = group.ages, y = shared, fill = is.pair)) +
  ...
  ggtitle("Fungi: Breastmilk") +
  geom_text(data = bm.res, size = 8, aes(x = group.ages, y = 17, label = sig.annot), inherit.aes = FALSE)

#ggsave("ms_bacteria_fungi_analysis/figures/shared_otus/shared_ITS2_bm_otus.pdf", device = "pdf", dpi = 300, width = 3, height = 4)
...
#ggsave("ms_bacteria_fungi_analysis/figures/shared_otus/shared_ITS2_ms_otus.pdf", device = "pdf", dpi = 300, width = 3, height = 4)
...
bm.ms <- filter(bm.most.shared, Freq > 1) # 6 OTUs
...
unique(bm.ms.p$Genus)
bm.ms.p$genus.p <- gsub("p__Ascomycota_unclassified", "p__Ascomycota", bm.ms.p$Genus)
...
#ggsave("ms_bacteria_fungi_analysis/figures/shared_otus/bm_ITS2_shared_linegraph.pdf", device = "pdf", dpi = 300, width = 8, height = 5.3)
...
st.ms <- filter(st.most.shared, Freq > 2) # 2 OTUs
...
# plot 
# shorten taxa names
unique(st.ms.p$Genus) # not needed
...
#ggsave("ms_bacteria_fungi_analysis/figures/shared_otus/prev_its2_st_shared_linegraph.pdf", device = "pdf", dpi = 300, width = 8, height = 5.3)
