...
# ITS2: plot sample depth and remove samples with low sample counts
######################
...
# load data
ps <- readRDS("ms_bacteria_fungi_analysis/Rdata/R_export/prev_ITS2_physeq_filtered.rds")
...
check <- filter(dat, remove == "Y") # 62 samples
...
#ggsave("ms_bacteria_fungi_analysis/figures/sequencing_depth/prev_ITS2_depth.pdf", device = "pdf", dpi = 300, width = 5, height = 5.3)
...
# get number of samples with no fungi ####
dat$Fungi.detected <- ifelse(dat$depth < 1000, "N", "Y")
dat$Fungi.detected <- factor(dat$Fungi.detected, levels = c("Y", "N"))

ggplot(dat, aes(x = group.ages.short, fill = Fungi.detected)) + 
  geom_bar(stat = "count") +
  scale_fill_manual("Fungi detected", values = c("black", "blue")) +
  theme_classic() +
  theme(axis.text.x = element_text(size = 14, face = "bold"),
        axis.text.y = element_text(size = 14, face = "bold"),
        axis.title.y = element_blank(),
...
#ggsave("ms_bacteria_fungi_analysis/figures/sequencing_depth/prev_ITS2_detected.pdf", device = "pdf", dpi = 300, width = 5, height = 5.3)

# save final physeq ####
#saveRDS(ps.final, "ms_bacteria_fungi_analysis/Rdata/R_export/ps_ITS2_for_analysis.rds")
